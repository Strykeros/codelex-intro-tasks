...
const fahrenheitToCelsius = function (temp: number) {
  let temperature = ((temp - 32) / 1.8).toFixed(1);

  if (temperature.includes(".0")) {
    temperature = temperature.replace(".0", "");
  }

  return temperature;
};
const celsiusToFahrenheit = function (temp: number) {
  let temperature = (temp * 1.8 + 32).toFixed(1);
