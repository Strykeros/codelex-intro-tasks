...
class Circle {
  ...
  area() {
    let areaVal = Math.PI * this.radius * this.radius;
    return areaVal.toFixed(2);
  }

  perimeter() {
    let radiusVal = 2 * Math.PI * this.radius;
    return radiusVal.toFixed(2);
  }
}

const c: Circle = new Circle(3);
